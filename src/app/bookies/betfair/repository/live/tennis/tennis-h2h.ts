...
// const TENNIS_URL =
//   'https://www.betfair.com/exchange/plus/es/tenis-apuestas-2/inplay';

const TENNIS_URL = 'https://www.betfair.com/exchange/plus/es/tenis-apuestas-2/';
...
export class TennisH2H {
  ...
  private static async createBetEvent(
    ...
    const _partials: PartialBet[] = await row.evaluate((row: Element) => {
      ...
      // @ts-ignore
      const buttons = [...row.querySelectorAll('.coupon-runners > div button')];
      ...
      return buttons.map((button, index) => {
        ...
        const odds = button.querySelector('label').textContent;
        return {
